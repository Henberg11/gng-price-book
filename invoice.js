...
// Loaded before the main script; the main script exposes window.PB and calls Invoices.open() from the header button.
(function () {
  ...
  let business = null, clients = {}, invoices = {}, loaded = false, editing = null;
  ...
  function host() { const v = S().viewEl(); v.innerHTML = ''; S().listEl().hidden = true; return v; }

  async function open() {
    const v = host(); v.innerHTML = '<div class="empty">Loading invoices…</div>';
    try { await load(); } catch (e) { v.innerHTML = `<div class="empty">Could not load: ${esc(e.message)}</div>`; return; }
    renderList();
  }
  ...
  function renderList() {
    ...
    v.innerHTML = `<div class="item"><div class="body">
      <h3>Invoices</h3>
      <div class="actions"><button type="button" id="invNewPI">New proforma</button><button type="button" id="invNewINV">New invoice</button><button type="button" class="ghost" id="invBiz">Business details</button></div>
  ...
  function newDoc(type) {
    return { number: nextNumber(type), type, date: today(), status: 'draft', client: { name: '', address: '', gstin: '', state: business.state || '', phone: '', email: '' },
      lines: [{ ref: '', name: '', hsn: '', qty: 1, unit: '' }], discount: { type: 'amt', value: 0 },
      gst: { mode: 'exclusive', rate: 0 }, notes: '', terms: business.terms, created: new Date().toISOString() };
  }

  function lineRow(l) {
    return `<tr><td><input type="text" class="l-ref" value="${esc(l.ref || '')}" placeholder="Ref" inputmode="numeric" style="width:64px"></td>
  ...
  function renderEditor(inv) {
    editing = inv; const v = host(); const isInv = inv.type === 'invoice';
    ...
    v.innerHTML = `<div class="item edit"><div class="body">
      ...
      <tbody>${inv.lines.map(lineRow).join('')}</tbody></table></div>
      <div class="actions"><button type="button" class="ghost small" id="iAdd">+ Add line</button></div>
      <div class="foot">Type a catalog reference in Ref and the name and sale price fill in. Leave Ref empty for delivery, customisation or any free line.</div>
    ...
    const wire = () => {
      v.querySelectorAll('#iLines .del').forEach(b => b.onclick = () => { b.closest('tr').remove(); totals(); });
    ...
    const totals = () => {
      ...
      v.querySelectorAll('#iLines tbody tr').forEach((tr, i) => { tr.querySelector('.l-total').textContent = c.lines[i] ? INR(c.lines[i].total) : '—'; });
      $('#iTotals').innerHTML = totalsHtml(c, true);
    };
    $('#iAdd').onclick = () => { $('#iLines tbody').insertAdjacentHTML('beforeend', lineRow({ qty: 1 })); wire(); $('#iLines tbody').lastElementChild.querySelector('input').focus(); };
    ...
    $('#iBack').onclick = renderList;
    $('#iSave').onclick = async () => { const inv = read(); $('#iSave').disabled = true; try { await persist(inv); msg('Saved'); } catch (e) { msg('Could not save: ' + e.message, true); } $('#iSave').disabled = false; };
  ...
  function read() {
    ...
    inv.lines = [...document.querySelectorAll('#iLines tbody tr')].map(tr => ({ ref: tr.querySelector('.l-ref').value.trim(), name: tr.querySelector('.l-name').value.trim(), hsn: tr.querySelector('.l-hsn').value.trim(), qty: S().num(tr.querySelector('.l-qty').value) ?? 1, unit: S().num(tr.querySelector('.l-unit').value) ?? 0 })).filter(l => l.name);
  ...
  function docHtml(inv) {
    ...
    const lines = c.lines.map((l, i) => `<tr><td class="n">${i + 1}</td><td>${esc(l.name)}${l.ref ? ` <span class="ref">Ref. ${esc(l.ref)}</span>` : ''}</td><td>${esc(l.hsn || '')}</td><td class="n">${l.qty % 1 ? l.qty : l.qty | 0}</td><td class="n">${INR(l.unit)}</td><td class="n">${INR(l.total)}</td></tr>`).join('');
    ...
    return `
      ...
      <div class="d-parties">
        <div><div class="d-lab">From</div><div class="d-name">${esc(b.name)}</div><div>${esc(b.address || '').replace(/\n/g, '<br>')}</div>${b.phone ? `<div>${esc(b.phone)}</div>` : ''}${b.email ? `<div>${esc(b.email)}</div>` : ''}</div>
        <div><div class="d-lab">${isInv ? 'Billed to' : 'Prepared for'}</div><div class="d-name">${esc(inv.client.name)}</div><div>${esc(inv.client.address || '').replace(/\n/g, '<br>')}</div>${inv.client.state ? `<div>${esc(inv.client.state)}</div>` : ''}${inv.client.gstin ? `<div>GSTIN ${esc(inv.client.gstin)}</div>` : ''}${inv.client.phone ? `<div>${esc(inv.client.phone)}</div>` : ''}</div>
      </div>
      ...
      <div class="d-foot">
        <div>${inv.terms ? `<div class="d-lab">Terms</div><div>${esc(inv.terms).replace(/\n/g, '<br>')}</div>` : ''}</div>
  ...
  window.Invoices = { open, calc, inWords };
})();
